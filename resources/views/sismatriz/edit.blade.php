...
@section('page-title', 'Editar Usuário - SisMatriz')
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-header bg-white border-bottom p-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 fw-bold">Editar Usuário SisMatriz Ticket: {{ $user->name }}</h5>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('sismatriz.update', $user->id) }}" method="POST" id="editSisMatrizUserForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                                    ...
                                    <option value="support" {{ old('role', $user->role) == 'support' ? 'selected' : '' }}>Suporte</option>
                                </select>
                                @error('role') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="col-md-6">
                                <label for="sacratech_id" class="form-label fw-medium">Sacratech ID</label>
                                <input type="number" class="form-control @error('sacratech_id') is-invalid @enderror" id="sacratech_id" name="sacratech_id" value="{{ old('sacratech_id', $user->sacratech_id) }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.getElementById('editSisMatrizUserForm');
