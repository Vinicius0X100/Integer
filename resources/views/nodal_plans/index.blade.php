...
<div class="container-fluid px-2 px-md-4 py-2">
    ...
    @if(!empty($apiError))
        <div class="card border-0 shadow-sm rounded-4 mb-4 bg-danger bg-opacity-10 border border-danger border-opacity-25 text-white p-3 p-md-4">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                <div>
                    <h5 class="fw-bold text-danger mb-1">
                        <i class="bi bi-exclamation-octagon-fill me-2"></i>Indisponibilidade do Nodal
                    </h5>
                    <p class="mb-0 text-white-50 small">{{ $apiError }}</p>
                </div>
    ...
    @endphp
    <div class="d-flex flex-nowrap overflow-auto pb-2 mb-4 gap-2 text-nowrap" style="scrollbar-width: thin;">
            ...
            <form action="{{ route('nodal-plans.index') }}" method="GET" class="row g-3 align-items-center">
                ...
                <div class="col-12 col-lg-2 d-flex gap-2">
                    ...
                    @if(request()->anyFilled(['search', 'status', 'unlimited', 'visibility', 'tab']))
                        <a href="{{ route('nodal-plans.index') }}" class="btn btn-outline-light rounded-circle flex-shrink-0" title="Limpar Filtros"><i class="bi bi-x-lg"></i></a>
                    @endif
                </div>
            </form>
        </div>
